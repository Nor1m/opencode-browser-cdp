...
export const GHOST_SOURCE = (config) => {
    ...
    const build = (host, root) => {
        ...
        const focus = add(root, "div", "focus", "position:fixed;left:0;top:0;width:0;height:0;opacity:0;border:3px solid #6ee7ff;border-radius:9px;box-shadow:0 0 0 2px rgba(5,13,24,.75),0 0 22px rgba(42,210,255,.65);transform:translate3d(0,0,0);transition:transform 180ms cubic-bezier(.2,.8,.2,1),width 180ms ease,height 180ms ease,opacity 100ms ease;box-sizing:border-box");
        ...
        const cursor = add(root, "div", "cursor", "position:fixed;left:0;top:0;width:24px;height:30px;opacity:0;transform:translate3d(0,0,0);transition:transform 180ms cubic-bezier(.2,.8,.2,1),opacity 100ms ease;filter:drop-shadow(0 3px 5px rgba(0,0,0,.35))");
        ...
        const hud = add(root, "aside", "hud", "position:fixed;top:16px;right:16px;width:min(330px,calc(100vw - 32px));overflow:hidden;color:#e8f5ff;background:rgba(5,13,24,.92);border:1px solid rgba(110,231,255,.42);border-radius:12px;box-shadow:0 14px 35px rgba(0,0,0,.32);backdrop-filter:blur(12px);font:12px/1.35 ui-monospace,SFMono-Regular,Consolas,monospace;box-sizing:border-box");
        const head = add(root, "div", "hud-head", "display:flex;align-items:center;justify-content:space-between;padding:10px 12px 8px;box-sizing:border-box", hud);
    ...
    const highlight = async () => {
        ...
        if (!position())
            return false;
        root.getElementById("focus-label").textContent = target.label;
        let scheduled = false;
