...
export const browserTool: any = tool({
  ...
  async execute(args) {
    try {
      ...
      if (args.port !== undefined) api.rememberPort(port)
      ...
      try {
        return await api.withPage(
          ...
          const result = await (async () => {
            ...
            case "eval": {
              if (!args.expression) return ok({ ok: false, error: "expression required" })
              ...
              if (args.selector) {
                ...
                const found = await page.evaluate((t, token) => {
                  ...
                  const el =
                    nodes.find(
                      (n) =>
                        (n.textContent || "").trim().toLowerCase() === want &&
                        (() => {
                    ...
                    nodes.find((n) => {
                      ...
                      const style = getComputedStyle(n)
                      return (
                        (n.textContent || "").trim().toLowerCase().includes(want) &&
                        rect.width > 0 &&
                        rect.height > 0 &&
                        style.display !== "none" &&
                        style.visibility !== "hidden"
            ...
            case "fill": {
              if (!args.selector) return ok({ ok: false, error: "selector required" })
              ...
              if (!editable) return ok({ ok: false, error: "element is not editable" })
              const focused = await focusTarget(args.selector)
              if (!focused) return ok({ ok: false, error: "element could not be focused" })
            ...
            case "select": {
              if (!args.selector) return ok({ ok: false, error: "selector required" })
              const value = args.value ?? args.text
              if (value == null) return ok({ ok: false, error: "value required" })
              await page.waitForSelector(args.selector, { timeout })
              await showTarget(args.selector)
            ...
            case "check": {
              ...
              if (!el) return ok({ ok: false, error: "element not found" })
              const checked = await page.evaluate((node) => !!(node as HTMLInputElement).checked, el)
              if (checked !== want) await el.click()
              const after = await page.evaluate((node) => !!(node as HTMLInputElement).checked, el)
              return ok({ ok: true, checked: after, url: page.url() })
            }
            case "press": {
              if (!args.key) return ok({ ok: false, error: "key required" })
              if (args.selector) {
                await page.waitForSelector(args.selector, { timeout })
                await showTarget(args.selector)
                if (!(await focusTarget(args.selector))) {
              ...
              await page.keyboard.press(args.key as Parameters<typeof page.keyboard.press>[0])
              await new Promise((r) => setTimeout(r, 200))
            ...
            case "screenshot": {
              ...
              await visual.withVisualsHidden(page, () =>
                page.screenshot({ path: file, fullPage: !!args.fullPage }),
              )
