...
可拖动 OpenCode 风格的紧凑 HUD 标题栏来移动窗口。输入附加指令并点击发送按钮，即可
将其加入下一次模型请求。最近的指令会显示为列表：绿色圆点表示待发送，灰色圆点表示
已发送；没有指令时不显示空状态。若要指定页面元素，请右键点击该元素并在注入的上下文菜单中选择 **看这里**。
此次右键操作会用插件菜单替代 Chrome 原生菜单。元素的选择器、文本和截断后的 HTML
只发送一次，随后自动清除。页面内容会明确标记为不可信数据，并作为合成的用户上下文
